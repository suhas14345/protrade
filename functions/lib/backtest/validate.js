...
const { computeMetrics, formatReport, cagr, maxDrawdown, sharpeRatio, dailyReturns, } = require('./metrics');
...
// 6. Synthetic generator is deterministic and produces valid OHLC ordering.
{
    ...
    const validOHLC = s1.every((b) => b.high >= Math.max(b.open, b.close) && b.low <= Math.min(b.open, b.close) && b.low > 0);
    check('synthetic OHLC ordering valid (high>=max, low<=min, low>0)', validOHLC);
    const validVol = s1.every((b) => b.volume > 0);
