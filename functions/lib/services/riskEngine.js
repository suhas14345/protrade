...
exports.riskEngineTask = void 0;
exports.doProcessRisk = doProcessRisk;
 ...
 */
async function doProcessRisk(dateId, jobId) {
    const db = getDb();
    ...
    for (const signal of signals) {
        ...
        if (approvedToday >= regime.maxNewPositions) {
        ...
        else if ((sectorCounts[sector] || 0) >= runtime_1.RISK_LIMITS.maxPerSectorPositions) {
            rejected = true;
            reason = `Sector cap reached for ${sector}.`;
        }
        else if (regime.marketState === 'HIGH_VOL' && approvedToday >= runtime_1.RISK_LIMITS.maxSameDirectionInHighVol) {
            rejected = true;
            ...
            rejected = true;
            reason = `Portfolio Heat too high (${(currentHeatR + signalHeatR).toFixed(1)}R > ${runtime_1.RISK_LIMITS.maxPortfolioHeatR}R).`;
        }
        // C. Update Signal
        const update = {
...
exports.riskEngineTask = functionsV1.https.onRequest(async (req, res) => {
