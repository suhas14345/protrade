...
const base = (over = {}) => (Object.assign({ symbol: 'TEST.NS', period: '2026Q1', filedAt: '2026-07-15' }, over));
describe('computeEarningsQuality', () => {
    it('returns UNKNOWN when no evaluable fields are present', () => {
        const res = (0, earningsQuality_1.computeEarningsQuality)(base());
    ...
    it('flags WATCH for large exceptional items (either sign)', () => {
        const gain = (0, earningsQuality_1.computeEarningsQuality)(base({ exceptionalItems: 80, pbt: 200 }));
        const loss = (0, earningsQuality_1.computeEarningsQuality)(base({ exceptionalItems: -80, pbt: 200 }));
    ...
    it('flags WATCH for an abnormally low effective tax rate', () => {
        const res = (0, earningsQuality_1.computeEarningsQuality)(base({ pbt: 200, tax: 10 }));
        expect(res.flags.map((f) => f.code)).toContain('LOW_EFFECTIVE_TAX');
    });
    it('flags a margin spike only when revenue did not grow', () => {
        const spikeNoGrowth = (0, earningsQuality_1.computeEarningsQuality)(base({
            revenueFromOps: 1010,
        ...
        const spikeWithGrowth = (0, earningsQuality_1.computeEarningsQuality)(base({
            revenueFromOps: 2000,
    ...
    it('flags FLAGGED (CRITICAL) for high promoter pledge', () => {
        const res = (0, earningsQuality_1.computeEarningsQuality)(base({ promoterPledge: 0.40, pbt: 100, tax: 25 }));
    ...
    it('flags FLAGGED (CRITICAL) for a promoter pledge increase', () => {
        const res = (0, earningsQuality_1.computeEarningsQuality)(base({ promoterPledge: 0.12, prevPromoterPledge: 0.04 }));
        expect(res.status).toBe('FLAGGED');
...
describe('nullFundamentalsSource', () => {
    it('is a fail-soft placeholder that yields no statement', async () => {
        expect(earningsQuality_1.nullFundamentalsSource.name).toBe('none');
