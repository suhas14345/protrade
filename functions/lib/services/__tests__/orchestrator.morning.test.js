...
describe('Orchestrator — Morning Fill Pipeline', () => {
    ...
    describe('processSymbolTask — taskSubType routing', () => {
        it('routes to morning handler when taskSubType is "morning"', async () => {
            ...
            mockFirestore.runTransaction = jest.fn(async (fn) => {
                return fn({
    ...
    describe('runMorningLogic', () => {
        it('enqueues tasks with taskSubType "morning" for each universe symbol', async () => {
            ...
            await (0, orchestrator_1.runMorningLogic)('2026-04-13', 'morning-job-1', 'nifty500');
            ...
            expect(mockFirestore.update).toHaveBeenCalledWith(expect.objectContaining({ 'counts.total': 3 }));
        });
        it('updates job to FAILED when enqueue throws', async () => {
            ...
            await expect((0, orchestrator_1.runMorningLogic)('2026-04-13', 'morning-job-1', 'nifty500'))
                .rejects.toThrow('Queue unavailable');
            expect(mockFirestore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'FAILED', errorMessage: 'Queue unavailable' }));
        });
    });
});
//# sourceMappingURL=orchestrator.morning.test.js.map