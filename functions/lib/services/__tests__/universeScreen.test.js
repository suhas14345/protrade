...
const universeScreen_1 = require("../universeScreen");
const runtime_1 = require("../../config/runtime");
const cfg = runtime_1.SCREEN_CONFIG;
// Build 252 bars: rising series ending near its high, with given latest close + volume.
function series(n, start, step, vol) {
...
describe('evaluateScreenStats', () => {
    it('passes a liquid, near-high, above-200DMA leader', () => {
        const bars = series(252, 100, 1, 1000000); // close ~351, rising → above 200-DMA, at highs
        const s = (0, universeScreen_1.evaluateScreenStats)(bars, cfg);
    ...
    it('fails on insufficient history', () => {
        const s = (0, universeScreen_1.evaluateScreenStats)(series(150, 100, 1, 1000000), cfg);
        expect(s.eligibleBase).toBe(false);
        expect(s.failedGate).toBe('history');
    });
    it('fails on price floor', () => {
        const bars = series(252, 5, 0.01, 100000000); // ~₹7.5, below ₹50
        const s = (0, universeScreen_1.evaluateScreenStats)(bars, cfg);
    ...
    it('fails when far below the 52-week high (downtrend)', () => {
        ...
        const up = series(200, 100, 2, 1000000); // 100..498
        const down = Array.from({ length: 52 }, (_, i) => ({ close: 498 - i * 6, volume: 1000000 })); // crashes
        const s = (0, universeScreen_1.evaluateScreenStats)([...up, ...down], cfg);
        expect(s.eligibleBase).toBe(false);
        expect(['below_200dma', 'far_from_high']).toContain(s.failedGate);
    });
});
//# sourceMappingURL=universeScreen.test.js.map