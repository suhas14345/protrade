...
jest.mock('../../config/runtime', () => ({
    ...
    SLIPPAGE_CONFIG: {
        ...
        BUCKET_A_MIN_VALUE: 50000000,
        BUCKET_B_MIN_VALUE: 10000000,
    },
}));
// ─── Drawdown Multiplier ───────────────────────────────────────────────────────
describe('computeDrawdownMultiplier', () => {
    ...
    function computeDrawdownMultiplier(account) {
        ...
        if (drawdownPct >= 0.20)
            return { multiplier: 0.0, shouldHalt: true, drawdownPct };
        if (drawdownPct >= DRAWDOWN_CONFIG.SEVERE_DD)
            return { multiplier: DRAWDOWN_CONFIG.SEVERE_MULT, shouldHalt: false, drawdownPct };
        if (drawdownPct >= DRAWDOWN_CONFIG.MODERATE_DD)
            return { multiplier: DRAWDOWN_CONFIG.MODERATE_MULT, shouldHalt: false, drawdownPct };
        if (drawdownPct >= DRAWDOWN_CONFIG.MILD_DD)
            return { multiplier: DRAWDOWN_CONFIG.MILD_MULT, shouldHalt: false, drawdownPct };
...
describe('computeSlippageBps', () => {
    function computeSlippageBps(liquidityBucket, marketState) {
        var _a, _b;
        const ranges = {
        ...
        const [lo, hi] = (_a = ranges[liquidityBucket]) !== null && _a !== void 0 ? _a : ranges['C'];
        const mult = (_b = regimeMults[marketState]) !== null && _b !== void 0 ? _b : 1.5;
...
describe('Portfolio constraints', () => {
    ...
    function checkPortfolio(openPositions, sector, signalHeatR, currentHeatR, clusterPositionCount, clusterHeatR, maxPositions, sessionApprovals) {
        const activeSectorCount = openPositions.filter(p => p.sector === sector).length;
        if (openPositions.length + sessionApprovals >= maxPositions)
            return { rejected: true, reason: 'Max positions' };
        if (activeSectorCount >= RISK_LIMITS.maxPerSectorPositions)
            return { rejected: true, reason: 'Sector cap' };
        if (currentHeatR + signalHeatR > RISK_LIMITS.maxPortfolioHeatR)
            return { rejected: true, reason: 'Portfolio heat' };
        if (clusterPositionCount >= CORR_CONFIG.MAX_POSITIONS_PER_CLUSTER)
            return { rejected: true, reason: 'Cluster cap' };
        if (clusterHeatR + signalHeatR > CORR_CONFIG.MAX_CLUSTER_RISK_R)
            return { rejected: true, reason: 'Cluster heat' };
    ...
    it('approves when all constraints pass', () => {
        const result = checkPortfolio([{ sector: 'BANK' }, { sector: 'IT' }], 'PHARMA', 0.5, 1.0, 0, 0, 10, 0);
        expect(result.rejected).toBe(false);
    });
});
//# sourceMappingURL=strategy.v22.test.js.map