...
describe('computeVDU (Volume Dry-Up Detection)', () => {
    function computeVDU(volumes, lookback = 20, threshold = 0.5) {
...
describe('computeGapRiskScore', () => {
     ...
     */
    function earningsProximityScore(daysToEarnings) {
    ...
    function gapHistoryScore(avgGapPct) {
        if (avgGapPct >= 0.04)
            return 0.3; // > 4% avg gap
        if (avgGapPct >= 0.02)
            return 0.15; // > 2% avg gap
        return 0;
    }
    function compositeGapRisk(daysToEarnings, avgGapPct) {
...
describe('ret60d calculation', () => {
    function computeRet(bars, period) {
...
describe('computeLiquidityBucket', () => {
    const BUCKET_A_MIN = 50000000; // 50M INR
    const BUCKET_B_MIN = 10000000; // 10M INR
    function getLiquidityBucket(medTradedValue20) {
        if (medTradedValue20 >= BUCKET_A_MIN)
            return 'A';
        if (medTradedValue20 >= BUCKET_B_MIN)
            return 'B';
        return 'C';
    }
    it('bucket A for large-cap (> 50M INR median traded value)', () => {
        expect(getLiquidityBucket(100000000)).toBe('A');
        expect(getLiquidityBucket(50000000)).toBe('A');
    });
    it('bucket B for mid-cap (10M–50M INR)', () => {
        expect(getLiquidityBucket(25000000)).toBe('B');
        expect(getLiquidityBucket(10000000)).toBe('B');
    });
    it('bucket C for small-cap (< 10M INR)', () => {
        expect(getLiquidityBucket(5000000)).toBe('C');
        expect(getLiquidityBucket(0)).toBe('C');
    });
});
//# sourceMappingURL=features.v22.test.js.map