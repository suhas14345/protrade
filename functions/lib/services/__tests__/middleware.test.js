...
describe('validateRequest', () => {
    it('accepts downloadReport with params (merged query+body path)', () => {
        expect((0, middleware_1.validateRequest)({ action: 'downloadReport', jobId: 'eod_2026-08-14_nifty50_1' }))
            .toEqual({ valid: true });
    });
    it('accepts a query-only shaped object (GET link)', () => {
        // Mimics { ...req.query } with no body — action still present.
        expect((0, middleware_1.validateRequest)({ action: 'downloadReport' }).valid).toBe(true);
    });
    it('rejects a missing action', () => {
        const r = (0, middleware_1.validateRequest)({});
        expect(r.valid).toBe(false);
        expect(r.error).toMatch(/action/i);
    });
    it('rejects an unknown action', () => {
        const r = (0, middleware_1.validateRequest)({ action: 'notARealAction' });
        expect(r.valid).toBe(false);
