...
describe('parseNseEquityCsv', () => {
    it('keeps only NSE cash equities and appends .NS', () => {
        ...
        const r = (0, marketdata_1.parseNseEquityCsv)(csv);
        expect(r).toEqual([
    ...
    it('excludes non-EQ / non-NSE rows (futures, indices)', () => {
        ...
        expect((0, marketdata_1.parseNseEquityCsv)(csv).map((e) => e.symbol)).toEqual(['INFY.NS']);
    });
    it('handles unquoted names containing commas via trailing-column parse', () => {
        ...
        expect((0, marketdata_1.parseNseEquityCsv)(csv)).toEqual([{ symbol: 'ACME.NS', name: 'Acme, Industries Ltd' }]);
    });
    it('skips tickers with spaces/odd characters', () => {
        const csv = [HEADER, '256265,0,NIFTY 50,NIFTY 50,0,,0,0,0,EQ,NSE,NSE'].join('\n');
        expect((0, marketdata_1.parseNseEquityCsv)(csv)).toEqual([]);
    });
});
//# sourceMappingURL=nseEquities.test.js.map