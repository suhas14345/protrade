...
describe('Strategy Service', () => {
    ...
    it('should generate a PullbackEOD signal when conditions match', async () => {
        ...
        mockFirestore.get
            ...
            // V2.4: Symbol mutex check (no existing approved signals)
            .mockResolvedValueOnce({ empty: true, docs: [] }) // existing signals query
            .mockResolvedValueOnce({
            empty: false,
            docs: [{ id: '20260321', data: () => mockBar }]
        })
            // V2.3: Event calendar — earnings staleness check + earnings + corporate + fnoBan
            .mockResolvedValueOnce({ empty: true, docs: [] }) // earnings staleness
            .mockResolvedValueOnce({ exists: false, data: () => ({}) }) // earnings/TCS.NS
            .mockResolvedValueOnce({ exists: false, data: () => ({}) }) // corporateActions
            .mockResolvedValueOnce({ exists: false, data: () => ({}) }); // fnoBans
        // Remaining gets (symbolMeta, calendar, corrTopN) fall through to default mock
