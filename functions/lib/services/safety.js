...
function checkSafety(lastBar, runDate) {
    var _a;
    ...
    const isLive = runtime_1.RUNTIME_CONFIG.MODE === 'LIVE' || runtime_1.RUNTIME_CONFIG.MODE === 'PAPER_LIVE';
    if (!isLive)
        return;
    if (lastBar) {
        let barTime;
        if (typeof ((_a = lastBar.timestamp) === null || _a === void 0 ? void 0 : _a.toMillis) === 'function') {
            barTime = lastBar.timestamp.toMillis();
        }
        else {
            barTime = new Date(lastBar.timestamp).getTime();
        }
        if (runDate) {
