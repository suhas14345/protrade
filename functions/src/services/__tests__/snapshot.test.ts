import { formatSnapshotText, DailySnapshot } from '../snapshot';
...
describe('formatSnapshotText', () => {
  it('renders an empty book cleanly', () => {
    const t = formatSnapshotText(base);
  ...
  it('lists active trades with P&L and stop', () => {
    const s: DailySnapshot = {
      ...base,
      positions: [{ symbol: 'BHEL.NS', strategy: 'SepaBreakoutEOD', qty: 214, entry: 413.5, current: 420, pnl: 1391, pnlPct: 1.6, stop: 384.55 }],
      critic: { severity: 'INFO', critical: 0, warn: 0 },
    };
    const t = formatSnapshotText(s);
  ...
  it('summarises the day activity and critic severity', () => {
    const s: DailySnapshot = {
      ...base,
      activity: { signals: 6, approved: 4, byStrategy: { SepaBreakoutEOD: 4, ATHPullbackEOD: 2 }, entryOrders: 4, entryFills: 0, exitFills: 1 },
      critic: { severity: 'CRITICAL', critical: 1, warn: 1 },
    };
    const t = formatSnapshotText(s);
  ...
  it('shows an em-dash for missing current/pnl', () => {
    const s: DailySnapshot = { ...base, positions: [{ symbol: 'X.NS', strategy: 'ATHPullbackEOD', qty: 10, entry: 100, current: null, pnl: null, pnlPct: null, stop: 90 }] };
    const t = formatSnapshotText(s);
