import { athPullbackSetup } from '../strategy';
...
describe('athPullbackSetup (ATH-Pullback entry gate)', () => {
  ...
  it('accepts a leader that pulled back into the 50-SMA buy-zone', () => {
    expect(athPullbackSetup(leader, CLOSE)).toBe(true);
  });

  it('rejects a stock still AT the highs (not pulled back)', () => {
    expect(athPullbackSetup(leader, 109)).toBe(false); // <3% off the high
  });

  it('rejects a deep breakdown (>15% off the high / below the 50-SMA)', () => {
    expect(athPullbackSetup(leader, 90)).toBe(false);
  });

  it('rejects when too extended above the 50-SMA (out of the buy-zone)', () => {
    expect(athPullbackSetup(leader, 106.5)).toBe(false); // dist50 ~6.5% > band
  });

  it('rejects an overbought pullback', () => {
    expect(athPullbackSetup({ ...leader, rsi14: 65 }, CLOSE)).toBe(false);
  });

  it('rejects an oversold pullback', () => {
    expect(athPullbackSetup({ ...leader, rsi14: 35 }, CLOSE)).toBe(false);
  });

  it('rejects a non-leader (weak RS rank)', () => {
    expect(athPullbackSetup({ ...leader, rsRank126: 80 }, CLOSE)).toBe(false);
  });

  it('rejects when the 200-SMA is not rising (no long-term uptrend)', () => {
    expect(athPullbackSetup({ ...leader, sma200Rising: false }, CLOSE)).toBe(false);
  });

  it('rejects a broken SMA stack (50 < 150)', () => {
    expect(athPullbackSetup({ ...leader, sma50: 90, sma150: 95 }, CLOSE)).toBe(false);
  });

  it('fails closed on a missing feature', () => {
    expect(athPullbackSetup({ ...leader, rsi14: undefined }, CLOSE)).toBe(false);
  });
  ...
  it('prefers the true ATH over the 52w high when present', () => {
    ...
    expect(athPullbackSetup({ ...leader, athHigh: 120 }, CLOSE)).toBe(true);
  });
  ...
  it('rejects when the true ATH makes the pullback too deep (>15%)', () => {
    ...
    expect(athPullbackSetup({ ...leader, athHigh: 130 }, CLOSE)).toBe(false);
  });

  it('falls back to the 52w high when athHigh is not yet tracked', () => {
    expect(athPullbackSetup({ ...leader, athHigh: undefined }, CLOSE)).toBe(true);
  });
});
