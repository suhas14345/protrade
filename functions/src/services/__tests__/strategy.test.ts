...
describe('Strategy Service', () => {
  ...
  function buildMockChain(overrides: {
    ...
    const features = {
      ema20: 100, ema50: 90, ema200: 80,
      low20: 95, rsi14: 45, atr14: 2,
      barsCount: 50, rsScore: 65, gapRiskScore: 0.1,
      vduActive: true,
      computedAt: admin.firestore.Timestamp.now(),
      ...overrides.features,
    };
    const regime = {
      marketState: 'TREND', tradeAllowed: true,
      riskMultiplier: 1.0, minSignalScore: 70,
      ...overrides.regime,
    };
    const account = {
    ...
    const bar = {
      id: '20260321',
      close: 98, high: 102, low: 95, volume: 2000000,
      timestamp: ts,
      ...overrides.bar,
    };

    const { mockFirestore: mf } = global as any;
    mf.get
  ...
  it('V3.2: rejects PullbackEOD when ema50 < ema200 (structural downtrend)', async () => {
    ...
    const approvedPullback = setCalls.find(
      (c: any[]) => c[0]?.strategy === 'PullbackEOD' && c[0]?.status === 'APPROVED'
    );
  ...
  it('V3.0: skips signal evaluation when RSI is unavailable (fail-closed)', async () => {
    ...
    const setCalls = mockFirestore.set.mock.calls;
    const anySignal = setCalls.find(
      (c: any[]) => c[0]?.strategy && c[0]?.direction
    );
