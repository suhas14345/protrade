...
import { recomputeAccountEquity } from '../portfolioEquity';

const fake = () => (globalThis as any).__fake as ReturnType<typeof makeFakeAdmin>;
...
describe('E2E ledger simulation — buy, mark, regime exit, sell', () => {
  ...
  async function recomputeAndAssert(dateId: string) {
    await recomputeAccountEquity(db(), dateId);
    const acct = await account();
    const positions = await openPositions();
    const trades = await allTrades();
    const deployed = positions.reduce((a: number, p: any) => a + Math.abs(p.avgEntryPrice * p.qty), 0);
    const openUnreal = positions.reduce((a: number, p: any) => a + (p.unrealizedPnl || 0), 0);
    const realized = trades.reduce((a: number, t: any) => a + (t.realizedPnl || 0), 0);
  ...
  it('BUY: doPlaceOrders creates an ACCEPTED entry order from an APPROVED signal', async () => {
    await db().doc('signals/20260105/items/GOLDBEES_20260105_MetalsRotation').set({
      ...
      riskApproval: { status: 'APPROVED', sizedQty: 700, riskAmount: 75_000 },
    });
  ...
  it('FILL: next-open fill opens the position and ties up capital', async () => {
    ...
    await doOpenFillSimulation('jobFill', '2026-01-06', 'GOLDBEES');
  ...
  it('REGIME EXIT: trend break queues a next-open EXIT order', async () => {
    ...
    await doManageTrades('20260108', 'jobMgr');
  ...
  it('SELL: exit fills, books realized P&L, and closes the position', async () => {
    ...
    await doOpenFillSimulation('jobExit', '2026-01-09', 'GOLDBEES');
